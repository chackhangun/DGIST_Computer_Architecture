...
int address_by_finding_far_from_main(std::vector<std::vector<std::string>> parsing_table, std::string str, std::vector<std::vector<std::vector<int>>> data_memory){
    ...
    for(int n = 0; n < parsing_table.size(); n++){
        ...
        if(parsing_table[n][0] == "la"){ //// lui,ori  instruction 몇개인지 구분하기 위해 위의 변수를 봐야함. 따라서 data memory 필요.
            ...
            int number = 0;            
            for (int i = 0; i < parsing_table.size(); n++){
        ...
        int byte_far_from_main = 4*(number_of_line - number_of_main - 1);
        ...
        if (data == true && text == true){ //instruction
            if(first_word.back() == ':'){
                ...
                if(first_word == "la"){
                    ...
                    bit_to_int_instruction = print_output(R,I,J, new_container, parsing_table, data_memory);
                    ...
                    if(downfour != "0000"){   ///////////ori 해야함.
                        ...
                        bit_to_int_instruction = print_output(R,I,J, second_new_container, parsing_table, data_memory);
                        std::pair<int, std::string> value = std::make_pair(mem_now_add_txt, bit_to_int_instruction);
                        txt_array.push_back(value);
                        std::cout << "memory address = " << value.first << " bit instruction = " << value.second << std::endl;
                    ...
                    std::string bit_to_int_instruction = print_output(R, I, J, container, parsing_table, data_memory);
                    std::pair<int, std::string> value = std::make_pair(mem_now_add_txt, bit_to_int_instruction);
                    txt_array.push_back(value);
                    std::cout << "memory address = " << value.first << " bit instruction = " << value.second << std::endl;
                }
